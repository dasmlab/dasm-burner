...
# Uses scripts/ci/remotessh (Go) when PREVIEW_PROXY_SSH_KEY is set — no system ssh.
set -euo pipefail

FQDN="${1:?usage: ensure-prod-cert.sh <fqdn>}"
if [[ "${FQDN}" != *.* ]]; then
  echo "ERROR: ensure-prod-cert.sh expects a FQDN (got ${FQDN})" >&2
  exit 1
fi
...
EOS
  ...
  echo "WARN: no PREVIEW_PROXY_SSH_KEY / ssh — skip HAProxy CERT ensure for ${FQDN}" >&2
  exit 0
fi
